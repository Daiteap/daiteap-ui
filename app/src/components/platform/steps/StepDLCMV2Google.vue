      ...
      <div class="row">
        ...
        >
          <NodeGroup
            ...
            :nodeTypes="$finalModel.gatewayCloud === 'google' && index < 1 ? [nodeTypes[0]] : nodeTypes"
  ...
  methods: {
    checkQuota() {
      ...
      for (const key in this.$finalModel) {
        if (Object.hasOwnProperty.call(this.$finalModel, key)) {
          if (this.$finalModel[key] != null && this.$finalModel[key].hasOwnProperty('nodes') && key != "google") {
            nodesCount += this.$finalModel[key].nodes.length;
          }
        }
      }

      nodesCount += this.form.google.nodes.length;
    ...
    evenControlPlaneNodes() {
      ...
      if (this.$finalModel.googleSelected){
        for (let i=0; i<this.form.google.nodes.length; i++){
          if (this.form.google.nodes[i].is_control_plane == true){
            cpNodes += 1;
          }
        }
      }
