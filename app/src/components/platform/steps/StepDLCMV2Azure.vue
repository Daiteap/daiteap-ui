      ...
      <div class="row">
        ...
        >
          <NodeGroup
            ...
            :nodeTypes="$finalModel.gatewayCloud === 'azure' && index < 1 ? [nodeTypes[0]] : nodeTypes"
  ...
  methods: {
    checkQuota() {
      ...
      for (const key in this.$finalModel) {
        if (Object.hasOwnProperty.call(this.$finalModel, key)) {
          if (this.$finalModel[key] != null && this.$finalModel[key].hasOwnProperty('nodes') && key != "azure") {
            nodesCount += this.$finalModel[key].nodes.length;
          }
        }
      }

      nodesCount += this.form.azure.nodes.length;
    ...
    evenControlPlaneNodes() {
      ...
      if (this.$finalModel.azureSelected){
        for (let i=0; i<this.form.azure.nodes.length; i++){
          if (this.form.azure.nodes[i].is_control_plane == true){
            cpNodes += 1;
          }
        }
      }
